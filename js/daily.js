...
function confirmDaily() {
  ...
  if (!val) return;
  const tasks = getDailyTasks();
  if (tasks.includes(val)) { alert('이미 있는 항목이에요!'); return; }
  input.value = '';
  tasks.push(val);
...
document.getElementById('dailyInput').addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); confirmDaily(); } });
